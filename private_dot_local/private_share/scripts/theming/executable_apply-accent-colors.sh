...
apply_accent_colors () {
    ...
    envsubst < "$XDG_CONFIG_HOME/cava/templates/config.envsubst" > "$XDG_CONFIG_HOME/cava/config"
    envsubst < "$XDG_CONFIG_HOME/cava/templates/config_mini.envsubst" > "$XDG_CONFIG_HOME/cava/config_mini"
}
...
apply_gtk_theme () {
    ...
    rm "$XDG_CONFIG_HOME/gtk-4.0/assets"
    ...
    lst_repos=("$XDG_DATA_HOME/dotfiles")
    lst_repos+=("$XDG_DATA_HOME/ansible-archlinux")
...
apply_sddm_theme () {
    ...
    < "$XDG_CONFIG_HOME/sddm-themes/theme-eucalyptus-drop.envsubst" \
